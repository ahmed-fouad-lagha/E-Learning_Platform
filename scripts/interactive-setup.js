...
async function interactiveSetup() {
  ...
  if (serviceRoleKey.trim()) {
    ...
    log.warning('Skipped Supabase Service Role Key (REQUIRED for admin functions)');
  }
  ...
  if (emailChoice === '1') {
    ...
    if (gmailUser.trim() && gmailPass.trim()) {
      envContent = envContent.replace(/SMTP_USER=.*/, `SMTP_USER=${gmailUser.trim()}`);
      envContent = envContent.replace(/SMTP_PASS=.*/, `SMTP_PASS=${gmailPass.trim()}`);
      envContent = envContent.replace(/SMTP_FROM=.*/, `SMTP_FROM=${gmailUser.trim()}`);
    ...
    if (outlookUser.trim() && outlookPass.trim()) {
      ...
      envContent = envContent.replace(/SMTP_USER=.*/, `SMTP_USER=${outlookUser.trim()}`);
      envContent = envContent.replace(/SMTP_PASS=.*/, `SMTP_PASS=${outlookPass.trim()}`);
      envContent = envContent.replace(/SMTP_FROM=.*/, `SMTP_FROM=${outlookUser.trim()}`);
    ...
    if (smtpHost.trim() && smtpUser.trim()) {
      envContent = envContent.replace(/SMTP_HOST=.*/, `SMTP_HOST=${smtpHost.trim()}`);
      envContent = envContent.replace(/SMTP_PORT=.*/, `SMTP_PORT=${smtpPort.trim() || '587'}`);
      envContent = envContent.replace(/SMTP_USER=.*/, `SMTP_USER=${smtpUser.trim()}`);
      envContent = envContent.replace(/SMTP_PASS=.*/, `SMTP_PASS=${smtpPass.trim()}`);
      envContent = envContent.replace(/SMTP_FROM=.*/, `SMTP_FROM=${smtpFrom.trim() || smtpUser.trim()}`);
      log.success('Custom SMTP configuration saved!');
    }
  } else {
    log.warning('Skipped email setup (REQUIRED for user registration/password reset)');
  }
  
  // 3. Social Login (Optional)
  log.title('3. Social Login Configuration (OPTIONAL)');
  const setupSocial = await question('Do you want to set up social login? (y/N): ');
  
  if (setupSocial.toLowerCase() === 'y' || setupSocial.toLowerCase() === 'yes') {
    // Google OAuth
    console.log('\n🔍 Google OAuth Setup:');
    ...
    if (googleClientId.trim() && googleSecret.trim()) {
      envContent = envContent.replace(/GOOGLE_CLIENT_ID=.*/, `GOOGLE_CLIENT_ID=${googleClientId.trim()}`);
      envContent = envContent.replace(/GOOGLE_CLIENT_SECRET=.*/, `GOOGLE_CLIENT_SECRET=${googleSecret.trim()}`);
    ...
    console.log('   4. Add redirect URI: http://localhost:3000/api/auth/callback/facebook');
    console.log('');
    
    const facebookId = await question('🔑 Facebook App ID (optional): ');
    const facebookSecret = await question('🔐 Facebook App Secret (optional): ');
    
    if (facebookId.trim() && facebookSecret.trim()) {
      envContent = envContent.replace(/FACEBOOK_CLIENT_ID=.*/, `FACEBOOK_CLIENT_ID=${facebookId.trim()}`);
      envContent = envContent.replace(/FACEBOOK_CLIENT_SECRET=.*/, `FACEBOOK_CLIENT_SECRET=${facebookSecret.trim()}`);
  ...
  const setupAnalytics = await question('Do you want to set up analytics? (y/N): ');
  
  if (setupAnalytics.toLowerCase() === 'y' || setupAnalytics.toLowerCase() === 'yes') {
    const gaId = await question('📊 Google Analytics Measurement ID (GA-XXXXXXXXXX): ');
    const mixpanelToken = await question('📈 Mixpanel Project Token (optional): ');
    
    if (gaId.trim()) {
      envContent = envContent.replace(/NEXT_PUBLIC_GA_MEASUREMENT_ID=.*/, `NEXT_PUBLIC_GA_MEASUREMENT_ID=${gaId.trim()}`);
      log.success('Google Analytics configured!');
    }
    
    if (mixpanelToken.trim()) {
      envContent = envContent.replace(/NEXT_PUBLIC_MIXPANEL_TOKEN=.*/, `NEXT_PUBLIC_MIXPANEL_TOKEN=${mixpanelToken.trim()}`);
