...
directories.forEach((dir) => {
  ...
  console.log(
    `${status} ${dir.name.padEnd(20)} (${fileCount} files) - ${dir.description}`
  );
});
...
const organizationTasks = [
  {
    task: "Documentation organized",
    check: () => dirExists("./docs/deployment") && dirExists("./docs/setup"),
  },
  {
    task: "Scripts categorized",
    check: () =>
      dirExists("./scripts/database") && dirExists("./scripts/setup"),
  },
  {
    task: "Tests organized",
    check: () => dirExists("./tests/auth") && dirExists("./tests/database"),
  },
  {
    task: "Components structured",
    check: () => dirExists("./components/ui") && dirExists("./components/auth"),
  },
  {
    task: "README updated",
    check: () =>
      fs.existsSync("./README.md") && fs.statSync("./README.md").size > 1000,
  },
  {
    task: "Package.json organized",
    check: () => {
      try {
        const pkg = JSON.parse(fs.readFileSync("./package.json", "utf8"));
        return (
          pkg.scripts && pkg.scripts["setup:env"] && pkg.scripts["test:all"]
        );
      } catch (e) {
        return false;
      }
